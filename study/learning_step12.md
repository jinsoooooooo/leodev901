...
**Q. SPA에서 헤더 메뉴는 onClick이 필요하니까 전부 클라이언트 컴포넌트로 만들어야 하나요?**
*   **A.** ❌ 아닙니다! 헤더를 통째로 클라이언트로 만들면 안 됩니다. 헤더도 **아일랜드 패턴**으로 분리합니다:
    - **서버**: 로고, 사이트 이름, `<Link>` 네비게이션 (Link는 HTML `<a>` 태그로 변환되므로 onClick이 아닙니다!)
    - **클라이언트**: 햄버거 메뉴 열기/닫기, 다크모드 토글 등 `useState`가 필요한 버튼만 분리
    ```tsx
    // Header.tsx (서버 컴포넌트 — 'use client' 없음!)
    import Link from 'next/link';
    import MobileMenuButton from './MobileMenuButton'; // 클라이언트 컴포넌트
    ...
    export default function Header() {
        return (
            <header>
                <h1>내 사이트</h1>                    {/* 서버 ✅ */}
                <nav>
                    <Link href="/">홈</Link>           {/* 서버 ✅ — <a> 태그! */}
                    <Link href="/blog">블로그</Link>    {/* 서버 ✅ */}
                </nav>
                <MobileMenuButton />                   {/* 이것만 클라이언트! */}
            </header>
        );
    }
    ```

**Q. onClick → API 호출 → 결과 렌더링, 이 흐름에서 서버/클라이언트 경계는?**
*   **A.** 버튼(onClick) + 결과 표시 영역 모두 **같은 클라이언트 컴포넌트** 안에 있어야 합니다. 결과 데이터를 `useState`로 관리하기 때문입니다:
    ```
    ┌── 페이지 전체 (서버 컴포넌트) ← SEO ✅ ──────┐
    │  헤더, 제목, 설명 등 → 서버에서 HTML 완성     │
    │                                              │
    │  ┌── 인터랙션 영역 (use client) ───────────┐  │
    │  │ 🔘 버튼 (onClick → API 호출)           │  │
    │  │ 📋 결과 표시 (useState로 관리)          │  │
    ...
    └──────────────────────────────────────────────┘
    ```
    💡 **판단 공식**: "이 UI가 유저 행동(클릭/입력)에 반응하는가?" → **Yes면 `use client`**, **No면 서버 컴포넌트(SEO)**
